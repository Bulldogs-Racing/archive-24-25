...
int main(void) {
    ...
    if (lgSpiOpen(chip, SPI_CHANNEL, SPI_SPEED) < 0) {
        perror("SPI setup");
        return 1;
    }
...
void initLCD() {
    lgGpioClaimOutput(chip, LCD_RS, 0);
    lgGpioClaimOutput(chip, LCD_E, 0);
    lgGpioClaimOutput(chip, LCD_D4, 0);
    lgGpioClaimOutput(chip, LCD_D5, 0);
    lgGpioClaimOutput(chip, LCD_D6, 0);
    lgGpioClaimOutput(chip, LCD_D7, 0);
