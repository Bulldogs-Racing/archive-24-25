...
print("PYTHON: DASHBOARD STARTING")
...
global_data["heartbeat_number"] = 0
global_data["drive_status"] = "NOT"

def is_stdin_empty():
    return not select.select([sys.stdin], [], [], 0)[0]
...
def process_line(dataline):
    ...
    #print("SID:" , sid)
    sys.stdout.flush()
    messagetype = MessageTypes(int(dataline[:sid]))
    messagedata = dataline[sid + 1:]
    
    # python doesnt have switch T_T
    if(messagetype == MessageTypes.HEARTBEAT):
        global_data["heartbeat_number"] = int(messagedata)
    elif(messagetype == MessageTypes.CANDRIVEORNOT):
        global_data["drive_status"] = messagedata

def load_data():
    #print("LOADING DATA")
    while not is_stdin_empty():
        #print("STDIN HAS DATA")
...
def get_ready():
    load_data()
    return f"{global_data['drive_status']} Ready to Drive [{global_data['heartbeat_number']}]"
